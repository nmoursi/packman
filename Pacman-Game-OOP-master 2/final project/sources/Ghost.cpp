...
Ghost::Ghost(int intialRow, int intialColumn, float size, Board *board) : Character(intialRow, intialColumn, size, board), mFreight(false), mGraph(nullptr), ai(nullptr),
																		  mIntialRow(intialRow), mIntialCol(intialColumn), homeTimer(0), inHome(true)
{
}
...
Ghost &Ghost::setResource(std::string r)
{
	mResource = r;
	return *this;
}
...
bool Ghost::reverse(Direction d) const
{
	...
	{
	case UP:
		return (mDirection == DOWN) ? true : false;
	case RIGHT:
		return (mDirection == LEFT) ? true : false;
	case DOWN:
		return (mDirection == UP) ? true : false;
	case LEFT:
		return (mDirection == RIGHT) ? true : false;
	default:
		return false;
	}
}
